...
JPFits::FitsFile::~FitsFile()
{
	delete DIMAGE, HEADERKEYS, HEADERKEYVALS, HEADERKEYCOMS;
}

JPFits::FitsFile::FitsFile(String^ FullFileName, String^ DiskUCharBufferName, TypeCode Precision, int NAxis1, int NAxis2)
{
...
JPFits::FitsFile::FitsFile(System::String ^FullFileName, array<int,1>^ Range, bool Populate_Header, bool Populate_Data, bool Do_Stats)
{
...
JPFits::FitsFile::FitsFile(String^ FullFileName, array<double,2>^ ImageData, bool Do_Stats)//create new fits image from new data array, using path_file filename
{
	int index;
	DIMAGE = ImageData;
	FULLFILENAME = FullFileName;
...
JPFits::FitsFile::FitsFile(String^ FullFileName, array<double>^ ImageData, bool Do_Stats)//create new fits image from new data array, using path_file filename
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
JPFits::FitsFile::FitsFile(System::String ^FullFileName, cli::array<unsigned __int32,2> ^ImageData, bool Do_Stats)
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
JPFits::FitsFile::FitsFile(System::String ^FullFileName, cli::array<unsigned __int32> ^ImageData, bool Do_Stats)
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
JPFits::FitsFile::FitsFile(System::String ^FullFileName, cli::array<__int32,2> ^ImageData, bool Do_Stats)
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
JPFits::FitsFile::FitsFile(System::String ^FullFileName, cli::array<unsigned __int16,2> ^ImageData, bool Do_Stats)
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
JPFits::FitsFile::FitsFile(System::String ^FullFileName, cli::array<__int16,2> ^ImageData, bool Do_Stats)
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
JPFits::FitsFile::FitsFile(String^ FullFileName, array<unsigned __int16>^ ImageData, bool Do_Stats)
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
JPFits::FitsFile::FitsFile(String^ FullFileName, array<__int16>^ ImageData, bool Do_Stats)
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
JPFits::FitsFile::FitsFile(System::String ^FullFileName, cli::array<__int8,2> ^ImageData, bool Do_Stats)
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
JPFits::FitsFile::FitsFile(System::String ^FullFileName, cli::array<unsigned __int8,2> ^ImageData, bool Do_Stats)
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
void JPFits::FitsFile::SetImage(array<double,2>^ ImageData, bool Do_Stats)//sometimes want to not bother with stats, so cant use property setter for images
{
	DIMAGE = ImageData;
	NAXIS1 = ImageData->GetLength(0);
	NAXIS2 = ImageData->GetLength(1);
...
void JPFits::FitsFile::WriteFile(System::String ^FullFileName, System::TypeCode Precision)
{
	FULLFILENAME = FullFileName;
	int index = FULLFILENAME->LastIndexOf("\\");
	FILENAME = FULLFILENAME->Substring(index+1);
...
void JPFits::FitsFile::WriteFile(System::TypeCode Precision)
{
	WRITEFILE(Precision);
}

void JPFits::FitsFile::WriteFileFromDiskBuffer(bool DeleteOrigDiskBuffer)
{
	if (FROMDISKBUFFER != true)
	{
		::MessageBox::Show("This FITS not created from ""JPFits::FitsFile(String^ FullFileName, String^ DiskUCharBufferName, TypeCode Precision, int NAxis1, int NAxis2)"" constructor.","Error");
		return;
	}
	FORMATHEADER();
	...
	array<unsigned char>^ CLine = gcnew array<unsigned char>(80);
	array<unsigned char>^ head = gcnew array<unsigned char>(LH*80);
	for (int i = 0; i < LH; i++)
	{
		CLine = StringToChars(HEADER[i]);
		for(int j = 0; j < 80; j++)
		{
			head[i*80+j] = CLine[j];
		}
...
JPFits::FitsFile::FitsFile(String^ FullFileName)
{
	int index;
	FULLFILENAME = FullFileName;
	index = FULLFILENAME->LastIndexOf("\\");
...
array<double,2>^ JPFits::FitsFile::ConvertTxtToDblArray(String^ FullFileName, bool IsPoorlyFormatted)
{
	if (IsPoorlyFormatted)
	{
		...
		for (int i = 0; i < rows; i++)
		{
			...
			for (int j = 0; j < line->Length; j++)
			{
				...
				if (ISNUMERIC(chstr) && !startindfnd)
				{
					startindfnd = true;
					numstartind = j;
					continue;
...
array<double,2>^ JPFits::FitsFile::GetSubImage(int X_Center, int Y_Center, int X_HalfWidth, int Y_HalfWidth)
{
...
array<double, 2>^ JPFits::FitsFile::GetSubImage(array<int, 1>^ Range)
{
